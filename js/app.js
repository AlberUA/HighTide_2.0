(() => {
    ...
    (() => {
        ...
        window.onload = function() {
            const buttonsNavCategories = document.querySelectorAll(".tabs-featured__title");
            if (buttonsNavCategories.length != 0) {
                const elementsItems = document.querySelectorAll(".categories-featured__item");
                elementsItems.forEach((el => {
                    el.classList.add("show");
                }));
                document.addEventListener("click", clickBlog);
                function clickBlog(e) {
                    const targetElement = e.target;
                    const activeElement = document.querySelector(".tabs-featured__title.active");
                    const elements = document.querySelectorAll(".categories-featured__item");
                    const elementType = targetElement.dataset.filter;
                    if (activeElement) activeElement.classList.remove("active");
                    targetElement.classList.add("active");
                    elements.forEach((element => {
                        !elementType || element.dataset.filter === elementType ? element.classList.add("show") : element.classList.remove("show");
                    }));
                }
            }
            ...
            if (input) input.oninput = function() {
                ...
                let newsItems = document.querySelectorAll(".item-blog__link-title");
                if (val != "") newsItems.forEach((function(elem) {
                    if (elem.innerText.toLowerCase().search(val) == -1) elem.closest(".blog__item").classList.add("hide"); else elem.closest(".blog__item").classList.remove("hide");
                })); else newsItems.forEach((function(elem) {
                    elem.closest(".blog__item").classList.remove("hide");
                }));
            };
        };
        window["FLS"] = true;
